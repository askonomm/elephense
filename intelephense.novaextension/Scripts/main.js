...
let langserver = null;
nova.commands.register('com.thorlaksson.intelephense.restartServer', (_workspace) => {
    if (langserver) {
        langserver.stop();
        langserver = null;
    }
...
nova.commands.register('com.thorlaksson.intelephense.resetWorkspaceStubs', (workspace) => {
    workspace.config.set('intelephense.workspace-stubs', defaults_1.defaultStubs);
});
const installIntelephense = () => {
    ...
    installProcess.onDidExit((exitStatus) => {
        ...
        if (0 !== exitStatus) {
            eventHandler === null || eventHandler === void 0 ? void 0 : eventHandler.emit('intelephenseFailedToInstall');
        }
        else {
            eventHandler === null || eventHandler === void 0 ? void 0 : eventHandler.emit('intelephenseInstalled');
        }
    });
    ...
    getIntelephensePath.onDidExit((exitStatus) => {
        if (0 === exitStatus) {
            if (hasCorrectVersion) {
                ...
                eventHandler === null || eventHandler === void 0 ? void 0 : eventHandler.emit('intelephenseUpToDate');
                return;
            }
        }
        console.info('Intelephense is either not installed or using the wrong version');
        eventHandler === null || eventHandler === void 0 ? void 0 : eventHandler.emit('intelephenseOutOfDate');
    });
...
class IntelephenseLanguageServer {
    constructor() {
        ...
        nova.config.observe('intelephense.language-server-path', (path, _) => {
            this.checkPath(path);
        });
        eventHandler === null || eventHandler === void 0 ? void 0 : eventHandler.on('startIntelephense', this.start);
        eventHandler === null || eventHandler === void 0 ? void 0 : eventHandler.on('intelephenseUpToDate', () => {
            this.start(nova.path.join(nova.extension.path, 'node_modules', 'intelephense', 'lib', 'intelephense.js'));
        });
        eventHandler === null || eventHandler === void 0 ? void 0 : eventHandler.on('intelephenseInstalled', () => {
            this.start(nova.path.join(nova.extension.path, 'node_modules', 'intelephense', 'lib', 'intelephense.js'));
        });
    }
    deactivate() {
        this.stop();
        eventHandler === null || eventHandler === void 0 ? void 0 : eventHandler.clear('startIntelephense');
        eventHandler === null || eventHandler === void 0 ? void 0 : eventHandler.clear('intelephenseUpToDate');
    }
    checkPath(providedPath) {
        if (!providedPath) {
            eventHandler === null || eventHandler === void 0 ? void 0 : eventHandler.emit('ensureBundledIntelephense');
        }
        else {
            eventHandler === null || eventHandler === void 0 ? void 0 : eventHandler.emit('startIntelephense', providedPath);
        }
    }
    start(path) {
        ...
        const client = new LanguageClient('intelephense', 'Intelephense Language Server', serverOptions, clientOptions);
        try {
            // Start the client
            client.start();
...
exports.activate = function () {
    ...
    eventHandler.on('intelephenseOutOfDate', () => {
        const needToInstall = new NotificationRequest('com.thorlaksson.intelephense.installing-intelephense');
    ...
    eventHandler.on('intelephenseInstalled', () => {
        const successfullyInstalled = new NotificationRequest('com.thorlaksson.intelephense.intelephense-installed');
    ...
    eventHandler.on('intelephenseFailedToInstall', () => {
        const failedToInstall = new NotificationRequest('com.thorlaksson.intelephense.intelephense-installation-failed');
    ...
    nova.workspace.config.onDidChange('intelephense.workspace-stubs', (newSettings, _oldSettings) => {
        const shouldReset = () => {
            if (newSettings.length !== defaults_1.defaultStubs.length) {
                return false;
            }
            newSettings.forEach((stub) => {
                if (!defaults_1.defaultStubs.includes(stub)) {
                    return false;
                }
            });
            return true;
